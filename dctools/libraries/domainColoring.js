/* 
 ...
 Written by Juan Carlos Ponce Campuzano, 12-Aug-2020
...
class domainColoring {
    ...
    constructor(func, size, t = 0, u = 0, n = 1) {
        ...
        //Variables to define the level curves for HSB scheme
        ...
        //Variables to define the level curves for RGB scheme
        //this.sharp = 1 / 3;
        this.nContour = 16;
    ...
    plotHSV(opt = 'Modulus') {
        ...
        for (let x = this.x; x < this.w; x++) {
            for (let y = this.x; y < this.h; y++) {
                ...
                let w = this.func.fn(c); //Evaluate function

                //noLoop();

                let h = this.funPhaseHSV(w.r, w.i);

                let v = this.funColorHSV(w.r, w.i);
    ...
    plotHSVReIm(opt = 'Re/Im') {
        ...
        for (let x = this.x; x < this.w; x++) {
            for (let y = this.x; y < this.h; y++) {
                ...
                let h = this.funPhaseReIm(w.r, w.i);
                let s = this.funColorSReIm(w.r, w.i);
                let v = this.funColorBReIm(w.r, w.i);
    ...
    plotHSVDisc(opt = 'Modulus') {
        ...
        for (let x = this.x; x < this.w; x++) {
            for (let y = this.x; y < this.h; y++) {
                ...
                let h = this.funPhaseHSVDiscrete(w.r, w.i);

                let v = this.funColorHSVDiscrete(w.r, w.i);
    ...
    plotHSVG() {
        ...
        for (let x = this.x; x < this.w; x++) {
            for (let y = this.x; y < this.h; y++) {
                ...
                let h = this.funPhaseHSVGrad(w.r, w.i);
                let s = pow(this.satHSVGrad(w.r, w.i), 1.3)
                let v = pow(this.valHSVGrad(w.r, w.i), 1.3);
    ...
    plotRGB(opt = 'Modulus') {
        ...
        for (let x = this.x; x < this.w; x++) {
            for (let y = this.y; y < this.h; y++) {
                ...
                let r = this.funPhaseRGB(w.r, w.i);

                let g = this.funColorRGB(w.r, w.i) - 50;

                let b = 180;

                //set(x, y, color(h, b * 0.8, 0.95));
    ...
    */
    ...
    plotHSL(opt = 'Modulus') {
        ...
        for (let x = this.x; x < this.w; x++) {
            for (let y = this.y; y < this.h; y++) {
                ...
                let h = this.funPhaseHSL(w.r, w.i);

                let s = 1;

                let l = this.funColorHSL(w.r, w.i);
    ...
    Here we need to define the level curves of phase and modulus
    Note: This is still not included. Too slow. :(
    ...
    plotHSLuv(opt = 'Modulus', minHue = 0, maxHue = 1) {
        ...
        for (let x = this.x; x < this.w; x++) {
            for (let y = this.y; y < this.h; y++) {
                ...
                let h = map(this.funPhaseHSLuv(w.r, w.i), 0, 1, minHue, maxHue);

                let s = 1;

                let l = this.funColorHSLuv(w.r, w.i);
    ...
    */
    ...
    update() {
        ...
        //this.func = this.verifyFunction(complex_expression(trimN(input.value())));
