...
@param  s  Function  This is the complex function f(z) 
@param  t  Number    Parameter t in [0,1]
@param  u  Number    Parameter u = exp(i * s), with s in [0, 2 pi]
@param  n  Number    Parameter n, an integer in [0, 30]

@return  Array   The parsed function of variable z 
...
function complex_expression(s, t, u, n) {
  ...
  //Check this clts.slidert
  let params = [{
    name: 't',
    defn: '{r:' + t + ',i:0}',
    //defn: '{r:def.slidert,i:0}', //'{r:par,i:0}',
    caption: (function (t) {
  ...
  {
    name: 'u',
    defn: '{r:Math.cos(' + u + '),i:Math.sin(' + u + ')}',
    //defn: '{r:Math.cos(def.slideru),i:Math.sin(def.slideru)}', //'{r:Math.cos(Math.PI*2*par),i:Math.sin(Math.PI*2*par)}',
    caption: (function (t) {
  ...
  },
  {
    name: 'n',
    defn: '{r:' + n + ',i:0}',
    //'{r:Math.floor(par*par*59 + 1.5),i:0}',//defn: '{r:def.slidern,i:0}', //'{r:Math.floor(par*par*59 + 1.5),i:0}',
    caption: (function (t) {
  ...
  let run = () => {
    ...
    if (result === null || state.j < state.tok.length)
      return null;
    let parameters = [];
    if (result.vars.hasOwnProperty('z\''))
      return null;
  ...
  let intpow = (y, c) => {
    if (c == 1)
      return y;
    if (c % 2 == 0)
      return square(intpow(y, c / 2));
    if (c % 3 == 0)
      return cube(intpow(y, c / 3));
    if (c % 5 == 0)
      return p5(intpow(y, c / 5));
  ...
  */
  let psymbol = (z, iters) => {
    ...
    if (end < 0) {
      ...
      //console.log("Enter an integer greater than or equal to 0");
      return NaN;
    } else if (end === 0) {
      return {
        r: 1,
  ...
  */
  let mobius = (z, a, b, c, d) => {
    ...
    if (cond.r === 0 && cond.i === 0) {
      ...
      //console.log("Sorry, not valid 😟 Recall that ad − bc ≠ 0");
      return NaN;
    }
    return div(u, l);
  };
