...
let funPhase = (x, y) => atan2(y, x);

let sharp = 1 / 3;
let nContour = 3.82;

let funColor = (x, y) => sharp * (log(sqrt(x * x + y * y)) / log(1.9) - floor(log(sqrt(x * x + y * y)) / log(1.9))) + 0.7; //sharp * (nContour * (PI - atan2(y, -x)) / (2 * PI) -  floor(nContour * (PI - atan2(y, -x)) / (2 * PI))) + 0.7;

function sat(x, y) {
  let satAux = log(sqrt(x * x + y * y)) / log(1.9);
