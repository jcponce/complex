...
// Now let's get the mouse position to show on screen
let mX, mY, yMouse, xMouse;
...
document.addEventListener('mousemove', event => {
    ...
    // Redefine values for shader
    yMouse = mapVal(mY, 0, HEIGHT, HEIGHT, 0) / HEIGHT * 2 - 1;
    xMouse = mapVal(mX, 0, WIDTH, 0, WIDTH) / WIDTH * 2 - 1;

    // Make sure values are square
    xMouse = xMouse * sc * WIDTH / HEIGHT;
    yMouse = yMouse * sc;

    $("#hud-left").text('x: '+ xMouse.toFixed(3));
    $("#hud-right").text(', y: '+ yMouse.toFixed(3));
...
function render() {
  ...
  // Update value of mouse position
  quad.material.uniforms.u_mX.value = xMouse;
  quad.material.uniforms.u_mY.value = yMouse;
